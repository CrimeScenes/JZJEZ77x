...
-- Sync mouse with target position (to simulate the crosshair on target)
...
-- TriggerBot action with better prediction
                        ...
                        if os.clock() - lastClickTime >= shared.Forbidden.TriggerBot.Settings.Cooldown then  
                            ...
                            if tool and tool:IsA("Tool") then
                                if not isIgnoringKnife() then
                                    -- Predictive shooting with remote events if applicable
...
-- Main loop to trigger the bot action (without drawing the FOV circle)
